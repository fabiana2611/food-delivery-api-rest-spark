...
import br.api.spark.dao.ReviewDao;
...
public class Sql2oReviewDao implements ReviewDao {
	...
	public void add(Review review) {
		...
		try (Connection con = sql2o.open()) {
			int id = (int) con.createQuery(sql, true).bind(review).executeUpdate().getKey();
			review.setId(id);
	...
	public List<Review> getAllReviewsByRestaurant(int restaurantId) {
		try (Connection con = sql2o.open()) {
			return con.createQuery("SELECT * FROM reviews WHERE restaurantId = :restaurantId")
					.addParameter("restaurantId", restaurantId).executeAndFetch(Review.class);
		}
	}
	...
	public List<Review> getAllReviewsByRestaurantSortedNewestToOldest(int restaurantId) {
		...
		for (Review review : unsortedReviews) {
			...
			if (i == unsortedReviews.size()) { // we need to do some funky business here to avoid an arrayindex
												// exception and handle the last element properly
				...
				if (review.compareTo(unsortedReviews.get(i)) == -1) { // first object was made earlier than second
																		// object
					sortedReviews.add(0, unsortedReviews.get(i));
					i++;
				} else if (review.compareTo(unsortedReviews.get(i)) == 0) {// probably should have a tie breaker here as
																			// they are the same.
					sortedReviews.add(0, unsortedReviews.get(i));
					i++;
				} else {
					sortedReviews.add(0, unsortedReviews.get(i)); // push the first object to the list as it is newer
